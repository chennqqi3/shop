		 ...
		 //遍历其他的非默认地址
		  ...
		  //遍历购物车中的商品 计算总金额 优惠金额  运费  最终要付的款项
		   ...
		   foreach($cat_info as $car){
				 ...
				 $data['goods_original_price']=$goods_info[0]['goods_original_price'];
				 $data['goods_num']=$car['goods_num'];
				 $data['goods_total_money']=$car['goods_num']*$goods_info[0]['goods_original_price'];
				
				 $total_money+=$data['goods_total_money'];
				
				 if($goods_info['goods_sale_price']>0){
				     $total_sale_money+=$car['goods_num']*($data['goods_original_price']-$goods_info['goods_sale_price']);
		   ...
		   //计算最后需要的钱数 总金额+运费-优惠金额
		    ...
		    $end_money=$total_money+$express_money-$total_sale_money;
