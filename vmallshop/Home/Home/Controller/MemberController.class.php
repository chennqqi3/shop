...
class MemberController extends Controller {
    public function index(){
		  ...
		  $payment_order=M("Order")->where(array('user_id'=>$user_id,'order_state'=>'已付款'))->select();
		  $payment_count=count($payment_order);
		  //查找已经收货的订单
		  $finish_order=M("Order")->where(array('user_id'=>$user_id,'order_state'=>'已收货'))->field('order_id')->select();
